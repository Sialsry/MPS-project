...
const dummyCategories = [
    { name: '팝', description: '팝 음악' },
    { name: '발라드', description: '발라드 음악' },
    { name: '힙합', description: '힙합/랩 음악' },
    { name: '록', description: '록 음악' },
    { name: 'EDM', description: '일렉트로닉 댄스 뮤직' },
    { name: '재즈', description: '재즈 음악' },
    { name: '클래식', description: '클래식 음악' }
];
...
async function seedDatabase() {
    try {
        ...
        const insertedMusics = await db.insert(musics).values(dummyMusics).returning();
            ...
            for (let musicIdx = 0; musicIdx < insertedMusics.length; musicIdx++) {
                ...
                if (music.grade <= companyIdx) {
                    ...
                    playRecords.push({
                        ...
                        use_price: '0',
                        played_at: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000), // 최근 30일 내
                        is_valid_play: true,
                    ...
                    if (Math.random() > 0.7) {
                        playRecords.push({
                            ...
                            use_case: music.inst ? '1' : '0',
                            use_price: '0',
                            played_at: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000),
                            is_valid_play: false,
