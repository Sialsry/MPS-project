import { pgTable, unique, bigserial, text, bigint, numeric, timestamp, boolean, integer, serial, varchar, foreignKey, date, pgEnum } from "drizzle-orm/pg-core"
...
export const rewardStatus = pgEnum("reward_status", ['successed', 'falied', 'pending'])
...
export const companyMusics = pgTable("company_musics", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	...
	foreignKey({
			...
			name: "company_musics_company_id_companies_id_fk"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.musicId],
			foreignColumns: [musics.id],
			name: "company_musics_music_id_musics_id_fk"
		}).onDelete("cascade"),
]);
