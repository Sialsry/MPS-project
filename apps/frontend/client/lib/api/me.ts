  // lib/api/me.ts
  import { MeOverview } from "@/lib/types/me";
  ...
  export async function fetchMeOverview(opts?: {
    ...
    console.info("[fetchMeOverview] url:", url, "mode:", useCookie ? "cookie" : "bearer");
  ...
  export async function updateMeProfileFormData(
    ...
    // ★ 서버 FileInterceptor('profile_image')와 필드명 일치
    ...
    const res = await fetch(`${base}/me/profile`, {
      ...
      headers: {
        Accept: "application/json",
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
        // Content-Type은 FormData가 boundary 포함해 자동 설정해야 함 → 직접 세팅하지 말 것
      },
      credentials: token ? "omit" : "include",
      body: fd,
    });
